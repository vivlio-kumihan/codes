...
for org_file in org_files:
    ...
    # 制作用の中間ファイルを生成させる。
    tmp_file = os.path.join('../csv/_tmp', filename)
    df.to_csv(tmp_file,
        encoding = "utf-8",
        index = False,
    ...
    for col in df_ruby:
        ...
        df_ruby[col] = [jaconv.kata2hira(ruby) for ruby in df_ruby[col]]
    ...
    # 写真の読み込みとSeries化
    ser_photo = pd.Series(glob.glob('../csv/_org/*.psd'))
    ...
    #####################
    # 解説用に氏名の中間ファイルを生成する。
    tmp_file = os.path.join('../csv/_tmp', f'中間ファイル_{filename}')
    df_name.to_csv(tmp_file,
        encoding = "utf-8",
        index = False,
        columns=['氏', '名'],
        sep = ',')
    ...
    for idx in range(len(df_name)):
        ...
        if not catch_idx.size == 0:
            ...
            os.rename(f'../csv/_org/{ser_photo_name}.psd', f'../csv/_gen/{name}.psd')
        else:
            shutil.copy2(f'../csv/_org/〓〓〓〓.psd', f'../csv/_gen/{name}.psd')
